...
public class CompoundCharSequence implements ComparableCharSequence {
    ...
    public CharSequence subSequence(int start, int end) {
        ...
        PartInfo partInfoEnd = getPartInfoForCharIndex(end - 1); // end is exclusive
        if (partInfoStart.partIndex == partInfoEnd.partIndex) {
            return parts[partInfoStart.partIndex].subSequence(start - partInfoStart.globalStart, end - partInfoStart.globalStart);
        }
        
        CharSequence[] resParts = new CharSequence[partInfoEnd.partIndex - partInfoStart.partIndex + 1];
        resParts[0] = parts[partInfoStart.partIndex].subSequence(start - partInfoStart.globalStart, parts[partInfoStart.partIndex].length());
