            ...
            for (querqy.model.Term term: position) {
                ...
                if (pos >= startPosition && pos < endPosition && charSequencesToDelete.contains(term.toCharSequenceWithField())) {
                    ...
                    if (hasRemaining) {
                        ...
                        if (toBeDeleted.contains(term)) { // same term twice - we keep a copy
                            hasRemaining = true;
                        } else {
                            toBeDeleted.add(term);
                        }
                    ...
                    hasRemaining = true;
                    // TODO: optimise: we can go to the next position in the sequence if
                    // we got here via pos < startPosition or pos >= endPosition (no need to check further terms at
                    // this position)
                }
            }
            pos++;
